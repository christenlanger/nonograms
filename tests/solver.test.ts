...
import { solveLine } from "../src/features/solver/utils";
...
describe("solveLine", () => {
  ...
  it("prevents blocks from merging via existing O", () => {
    const result = solveLine(["O", "O", "?", "O", "O"], [2, 2]);
    expect(result.cells.join("")).toBe("OOXOO");
  });

  it("fills entire row with X if there are 0 hints", () => {
    const result = solveLine(Array(5).fill("?"), [0]);
  ...
  it("hints exceed the length of the line", () => {
    const result = solveLine(["?", "?", "?", "?"], [1, 1, 1, 1]);
